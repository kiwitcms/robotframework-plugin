...
class zealandLibrary:
    ...
    def set_categories(self):
        categories= self.rpc_client.Category.filter({'product_id':self.products.get(self.product)})
        for category in categories:
            self.categories[category['name']]= category['id']
    
    def set_builds(self):
        builds= self.rpc_client.Build.filter({'product_id':self.products.get(self.product)})
        for build in builds:
    ...
    def create_test_run(self,product,test_executor_email,notes=''):
        ...
        self.rpc_client.TestRun.add_tag(test_run['id'],'Il nodo Jenkins che ha eseguito la run : {0}'.format(self.node))
    ...
    def check_test_case_and_update(self,test_tags,test_status,test_name,test_doc,test_message):
        ...
        if(test_case_run):
            ...
            is_already_proposed=self.rpc_client.TestCase.filter({'plan':self.plan_id,'id': test_id, 'case_status': self.test_case_statuses.get('PROPOSED')})
    ...
    def update_testExecutions(self,updated_test):
        test_list=[t for t in self.rpc_client.TestRun.get_cases(self.test_run_id) if t['execution_id'] == updated_test['execution_id']]
        x=list({v['id']:v for v in test_list}.values())
        if(x == []):
            self.rpc_client.TestExecution.update(updated_test['execution_id'],{'status':self.test_exec_statuses.get('WAIVED')})
            self.rpc_client.TestExecution.add_comment(updated_test['execution_id'],'Test not executed and updated with Waived status')
        else:
            if(x[0]['status']  == 'IDLE'):
                self.rpc_client.TestExecution.update(x[0]['execution_id'],{'status':self.test_exec_statuses.get(updated_test['status'])})
                message= 'Test executed with status:' + updated_test['status'] + '\n' + updated_test['message'] 
                self.rpc_client.TestExecution.add_comment(x[0]['execution_id'],message)
                #Find a better way to handle it
                if updated_test['jid']: self.rpc_client.TestCase.add_tag(x[0]['id'],updated_test['jid'])

    def close_test_run(self,jenkins_tag):
        manual_test=[t for t in self.rpc_client.TestRun.get_cases(self.test_run_id) if t['is_automated']  is not True]
        if not manual_test:     
            stop_date =  datetime.datetime.strptime(self.NOW, '%Y-%m-%d %H:%M:%S')
            if jenkins_tag is not None: self.rpc_client.TestRun.add_tag(self.test_run_id,jenkins_tag)
            ...
            for test in manual_test:
                self.rpc_client.TestExecution.add_comment(test['execution_id'],'Test da eseguire manualmente')
                self.rpc_client.TestExecution.update(test['execution_id'],{'status':self.test_exec_statuses.get('RUNNING')})
